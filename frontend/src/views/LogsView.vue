...
const filters=reactive<any>({range:null,success:null,request_kind:null,unified_model:null,provider_instance_id:null,upstream_model_id:null,inbound_protocol:null,api_token_id:null,min_cost:null,max_cost:null})
...
function resetFilters(){Object.assign(filters,{range:null,success:null,request_kind:null,unified_model:null,provider_instance_id:null,upstream_model_id:null,inbound_protocol:null,api_token_id:null,min_cost:null,max_cost:null});page.value=1;void load()}
...
const columns:any[]=[{title:'请求 ID',key:'request_id',width:220,ellipsis:{tooltip:true}},{title:'调用类型',key:'request_kind',width:100,render:(r:any)=>h(NTag,{type:r.request_kind==='auxiliary'?'info':'default'},{default:()=>r.request_kind==='auxiliary'?'辅助':'主调用'})},{title:'协议',key:'inbound_protocol',width:150},{title:'供应商',key:'provider_name',width:160,render:(r:any)=>r.provider_name||'-'},{title:'上游模型',key:'upstream_model_name',width:200,ellipsis:{tooltip:true},render:(r:any)=>r.upstream_model_name||'-'},{title:'统一模型',key:'unified_model',width:180},{title:'客户端名称',key:'api_token_name',width:160,ellipsis:{tooltip:true},render:clientName},{title:'状态',key:'success',width:80,render:(r:any)=>h(NTag,{type:r.success?'success':'error'},{default:()=>r.success?'成功':'失败'})},{title:'延迟',key:'latency',width:110,render:(r:any)=>`${Number(r.latency_ms||0).toFixed(1)} ms`},{title:'时间（UTC+8）',key:'started_at',width:190,render:(r:any)=>formatChinaDateTime(r.started_at)},{title:'操作',key:'actions',width:80,fixed:'right',render:(r:any)=>h(NButton,{size:'small',onClick:()=>openDetail(r)},{default:()=> '详情'})}]
