  ...
  <n-space vertical size="large">
    ...
    <n-card title="候选上游模型">
      ...
      <n-form inline>
        ...
        <n-form-item label="权重"><n-input-number v-model:value="candidateForm.weight" :min="0"/></n-form-item><n-form-item label="启用"><n-switch v-model:value="candidateForm.enabled"/></n-form-item>
      </n-form>
      ...
      <n-alert type="info" style="margin-top:12px">拖动候选行即可调整优先级，越靠上越优先。</n-alert>
      <n-data-table data-testid="candidate-priority-table" style="margin-top:12px" :columns="candidateColumns" :data="selectedModel?.candidates||[]" :loading="candidateReordering" :row-props="candidateRowProps"/>
    </n-card>
  </n-space>
</template>
...
const message=useMessage();const unified=ref<any[]>([]);const upstream=ref<any[]>([]);const editingModelId=ref<number|null>(null);const selectedUnifiedId=ref<number|null>(null);const editingCandidateId=ref<number|null>(null);const draggedCandidateId=ref<number|null>(null);const candidateReordering=ref(false)
...
const candidateForm=reactive<any>({upstream_model_id:null,weight:100,enabled:true,override_input:[],override_output:[]})
...
function resetCandidate(){editingCandidateId.value=null;Object.assign(candidateForm,{upstream_model_id:null,weight:100,enabled:true,override_input:[],override_output:[]})}
function editCandidate(row:any){editingCandidateId.value=row.id;Object.assign(candidateForm,{upstream_model_id:row.upstream_model_id,weight:row.weight,enabled:row.enabled,override_input:[...(row.capability_overrides?.input||[])],override_output:[...(row.capability_overrides?.output||[])]})}
async function saveCandidate(){if(!selectedUnifiedId.value||!candidateForm.upstream_model_id)return message.warning('请选择统一模型和上游模型');const overrides:any={};if(candidateForm.override_input.length)overrides.input=[...candidateForm.override_input];if(candidateForm.override_output.length)overrides.output=[...candidateForm.override_output];const payload={upstream_model_id:candidateForm.upstream_model_id,weight:candidateForm.weight,enabled:candidateForm.enabled,capability_overrides:overrides};try{if(editingCandidateId.value)await patchJson(`/api/admin/unified-models/${selectedUnifiedId.value}/candidates/${editingCandidateId.value}`,payload);else await postJson(`/api/admin/unified-models/${selectedUnifiedId.value}/candidates`,payload);resetCandidate();await load()}catch(error){message.error(String(error))}}
...
async function reorderCandidates(targetId:number){const model=selectedModel.value;const sourceId=draggedCandidateId.value;draggedCandidateId.value=null;if(!model||sourceId===null||sourceId===targetId)return;const ordered=[...model.candidates];const sourceIndex=ordered.findIndex((row:any)=>row.id===sourceId);const targetIndex=ordered.findIndex((row:any)=>row.id===targetId);if(sourceIndex<0||targetIndex<0)return;const [moved]=ordered.splice(sourceIndex,1);ordered.splice(targetIndex,0,moved);model.candidates=ordered.map((row:any,index:number)=>({...row,priority:index+1}));candidateReordering.value=true;try{await patchJson(`/api/admin/unified-models/${model.id}/candidates/reorder`,{ids:ordered.map((row:any)=>row.id)});message.success('候选优先级已更新')}catch(error){message.error(String(error));await load()}finally{candidateReordering.value=false}}
function candidateRowProps(row:any){return {draggable:!candidateReordering.value,class:`sortable-row${draggedCandidateId.value===row.id?' is-dragging':''}`,onDragstart:(event:DragEvent)=>{draggedCandidateId.value=row.id;if(event.dataTransfer){event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',String(row.id))}},onDragover:(event:DragEvent)=>{event.preventDefault();if(event.dataTransfer)event.dataTransfer.dropEffect='move'},onDrop:(event:DragEvent)=>{event.preventDefault();void reorderCandidates(row.id)},onDragend:()=>{draggedCandidateId.value=null}}}
...
const candidateColumns:any[]=[{title:'优先级',key:'priority',width:76,align:'center',render:(_:any,index:number)=>h('span',{class:'priority-order',title:'拖动调整优先级','aria-label':`当前优先级 ${index+1}，拖动可调整`},[h('span',{class:'drag-handle','aria-hidden':'true'},'⠿'),h('span',{},String(index+1))])},{title:'供应商',key:'provider',render:(r:any)=>r.provider_instance?.name||'-'},{title:'上游模型',key:'upstream',render:(r:any)=>r.upstream_model?.model_id||r.upstream_model_id},{title:'权重',key:'weight'},{title:'能力覆盖',key:'capability_overrides',render:(r:any)=>`${(r.capability_overrides?.input||[]).join(', ')||'继承'} → ${(r.capability_overrides?.output||[]).join(', ')||'继承'}`},{title:'状态',key:'enabled',render:(r:any)=>r.enabled?'启用':'停用'},{title:'操作',key:'actions',render:(r:any)=>h(NSpace,{}, {default:()=>[h(NButton,{size:'small',onClick:()=>editCandidate(r)},{default:()=> '编辑'}),h(NButton,{size:'small',type:'error',onClick:()=>deleteCandidate(r)},{default:()=> '删除'})]})}]
onMounted(load)
</script>
<style scoped>
