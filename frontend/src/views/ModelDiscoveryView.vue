<template>
  <n-space vertical size="large">
    ...
    <n-card title="模型列表">
      <n-alert type="info" style="margin-bottom:12px">默认显示全部供应商，可跨供应商勾选模型并批量配置；顶部供应商选择仅用于添加、拉取和同步模型。</n-alert>
      <n-space justify="space-between" wrap align="center" style="margin-bottom:12px">
        <n-space align="center"><span>列表范围</span><n-select data-testid="model-list-provider-filter" v-model:value="modelProviderFilter" :options="modelProviderFilterOptions" style="width:240px" @update:value="changeModelProviderFilter"/><n-input data-testid="model-id-search" v-model:value="modelSearchQuery" clearable placeholder="搜索模型 ID" style="width:240px"/><n-tag v-if="checkedRowKeys.length" type="info">已选 {{checkedRowKeys.length}} 个模型</n-tag></n-space>
        <n-space><n-button data-testid="bulk-configure-button" :disabled="!checkedRowKeys.length" @click="openBulkConfigure">批量配置</n-button><n-button :disabled="!checkedRowKeys.length" @click="bulk('enable')">批量启用</n-button><n-button :disabled="!checkedRowKeys.length" @click="bulk('disable')">批量停用</n-button><n-button type="error" :disabled="!checkedRowKeys.length" @click="bulk('delete')">批量删除</n-button></n-space>
      </n-space>
      <n-card v-if="bulkConfigureVisible" data-testid="bulk-configure-panel" embedded size="small" class="bulk-configure-card" :title="`批量配置 ${checkedRowKeys.length} 个上游模型`">
        <n-alert type="warning" style="margin-bottom:12px">保存后，以下能力、限制、价格和标签会覆盖所有已选模型。</n-alert>
        <n-grid responsive="screen" :cols="'1 m:2 l:3'" :x-gap="16">
          <n-form-item-gi label="输入能力"><capability-checkbox-group v-model="bulkForm.input_caps" :options="inputCapabilityOptions"/></n-form-item-gi><n-form-item-gi label="输出能力"><capability-checkbox-group v-model="bulkForm.output_caps" :options="outputCapabilityOptions"/></n-form-item-gi>
          <n-form-item-gi label="上下文长度"><n-input-number v-model:value="bulkForm.context_window" :min="1" clearable/></n-form-item-gi><n-form-item-gi label="最大输出"><n-input-number v-model:value="bulkForm.max_output_tokens" :min="1" clearable/></n-form-item-gi>
          <n-form-item-gi label="输入价格/百万"><n-input-number v-model:value="bulkForm.input_price" :min="0" clearable/></n-form-item-gi><n-form-item-gi label="输出价格/百万"><n-input-number v-model:value="bulkForm.output_price" :min="0" clearable/></n-form-item-gi><n-form-item-gi label="缓存输入价格"><n-input-number v-model:value="bulkForm.cached_input_price" :min="0" clearable/></n-form-item-gi>
          <n-form-item-gi label="标签"><n-input v-model:value="bulkForm.tags" placeholder="free, fast"/></n-form-item-gi>
        </n-grid>
        <n-space><n-button data-testid="save-bulk-configuration" type="primary" :loading="bulkSaving" @click="saveBulkConfiguration">覆盖已选模型</n-button><n-button @click="bulkConfigureVisible=false">取消</n-button></n-space>
      </n-card>
      <n-empty v-if="!loading&&!filteredModels.length" :description="modelSearchQuery.trim()?'未找到匹配的模型 ID':'当前范围暂无模型，可同步或手工添加'"/>
      <n-data-table v-else data-testid="upstream-model-table" :columns="columns" :data="filteredModels" :loading="loading" :row-key="(row:any)=>row.id" v-model:checked-row-keys="checkedRowKeys" :pagination="{pageSize:20}" :scroll-x="1490"/>
    </n-card>
  </n-space>
</template>
...
const message=useMessage();const providers=ref<any[]>([]);const models=ref<any[]>([]);const providerId=ref<number|null>(null);const modelProviderFilter=ref<number|'all'>('all');const modelSearchQuery=ref('');const loading=ref(false);const syncing=ref(false);const testing=ref(false);const pulling=ref(false);const probingId=ref<number|null>(null);const syncResult=ref<any>();const checkedRowKeys=ref<Array<number|string>>([]);const editingId=ref<number|null>(null);const remoteCatalog=ref<any[]>([]);const selectedRemoteModelId=ref<string|null>(null);const capabilitySource=ref('');const capabilityConfidence=ref<'high'|'medium'|'low'>('low');const bulkConfigureVisible=ref(false);const bulkSaving=ref(false)
...
const bulkForm=reactive<any>({input_caps:['text'],output_caps:['text'],context_window:null,max_output_tokens:null,input_price:null,output_price:null,cached_input_price:null,tags:''})
const providerOptions=computed(()=>providers.value.map(p=>({label:`${p.name} · ${p.protocol_type}`,value:p.id})))
const modelProviderFilterOptions=computed(()=>[{label:'全部供应商',value:'all'},...providers.value.map(p=>({label:p.name,value:p.id}))])
const filteredModels=computed(()=>{const query=modelSearchQuery.value.trim().toLocaleLowerCase();return models.value.filter(row=>(modelProviderFilter.value==='all'||row.provider_instance_id===modelProviderFilter.value)&&(!query||String(row.model_id||'').toLocaleLowerCase().includes(query)))})
...
async function loadModels(){loading.value=true;try{models.value=await getJson('/api/admin/upstream-models');checkedRowKeys.value=[]}finally{loading.value=false}}
function changeModelProviderFilter(){checkedRowKeys.value=[];bulkConfigureVisible.value=false}
...
async function bulk(action:string){try{await postJson('/api/admin/upstream-models/bulk',{ids:checkedRowKeys.value.map(Number),action});await loadModels()}catch(error){message.error(String(error))}}
function openBulkConfigure(){const first=models.value.find(row=>checkedRowKeys.value.includes(row.id));if(!first)return;Object.assign(bulkForm,{input_caps:[...(first.input_capabilities_json||[])],output_caps:[...(first.output_capabilities_json||[])],context_window:first.context_window??null,max_output_tokens:first.max_output_tokens??null,input_price:first.input_price??null,output_price:first.output_price??null,cached_input_price:first.cached_input_price??null,tags:(first.tags_json||[]).join(', ')});bulkConfigureVisible.value=true}
async function saveBulkConfiguration(){if(!checkedRowKeys.value.length)return;bulkSaving.value=true;try{await postJson('/api/admin/upstream-models/bulk',{ids:checkedRowKeys.value.map(Number),action:'configure',configuration:{input_capabilities_json:[...bulkForm.input_caps],output_capabilities_json:[...bulkForm.output_caps],context_window:bulkForm.context_window,max_output_tokens:bulkForm.max_output_tokens,input_price:bulkForm.input_price,output_price:bulkForm.output_price,cached_input_price:bulkForm.cached_input_price,tags_json:csv(bulkForm.tags)}});bulkConfigureVisible.value=false;message.success('已批量更新上游模型配置');await loadModels()}catch(error){message.error(String(error))}finally{bulkSaving.value=false}}
...
const cellEllipsis={tooltip:true}
const columns:any[]=[
  {type:'selection',width:44},
  {title:'供应商',key:'provider_name',width:110,ellipsis:cellEllipsis},
  {title:'模型 ID',key:'model_id',width:170,ellipsis:cellEllipsis},
  {title:'显示名',key:'display_name',width:180,ellipsis:cellEllipsis},
  {title:'输入 / 输出能力',key:'caps',width:240,ellipsis:cellEllipsis,render:(r:any)=>`${(r.input_capabilities_json||[]).join(', ')} → ${(r.output_capabilities_json||[]).join(', ')}`},
  {title:'上下文 / 最大输出',key:'limits',width:150,ellipsis:cellEllipsis,render:(r:any)=>`${r.context_window??'未知'} / ${r.max_output_tokens??'未知'}`},
  {title:'价格',key:'price',width:90,ellipsis:cellEllipsis,render:(r:any)=>`${r.input_price??'-'} / ${r.output_price??'-'}`},
  {title:'远端状态',key:'remote_status',width:105,render:(r:any)=>h(NTag,{type:r.remote_status==='missing'||r.remote_status==='probe_failed'?'error':r.remote_status==='available'?'success':'default'},{default:()=>({missing:'远端不可用',probe_failed:'检测失败',available:'可用'} as any)[r.remote_status]||r.remote_status})},
  {title:'引用',key:'reference_count',width:60},
  {title:'状态',key:'enabled',width:60,render:(r:any)=>r.enabled?'启用':'停用'},
  {title:'操作',key:'actions',width:280,fixed:'right',render:(r:any)=>h(NSpace,{wrap:false},{default:()=>[h(NButton,{size:'small',loading:probingId.value===r.id,onClick:()=>probe(r)},{default:()=> '最小检测'}),h(NButton,{size:'small',onClick:()=>edit(r)},{default:()=> '编辑'}),h(NButton,{size:'small',onClick:()=>toggle(r)},{default:()=>r.enabled?'停用':'启用'}),h(NButton,{size:'small',type:'error',disabled:r.reference_count>0,onClick:()=>remove(r)},{default:()=> '删除'})]})}
]
...
.bulk-configure-card{margin-bottom:12px}
</style>
