...
async function saveStartup(enabled:boolean){startup.saving=true;try{const value:any=await patchJson('/api/admin/settings/startup',{enabled});Object.assign(startup,value);message.success(enabled?'开机自启动已开启':'开机自启动已关闭')}catch(error){message.error(String(error))}finally{startup.saving=false}}
