<template>
  <n-space vertical size="large">
    ...
    <n-card title="辅助模型池">
      ...
      <n-alert type="info" style="margin:12px 0">拖动辅助模型行即可调整优先级，越靠上越优先。</n-alert>
      <n-data-table data-testid="aux-model-priority-table" :columns="modelColumns" :data="models" :loading="modelReordering" :row-props="modelRowProps" />
    </n-card>
    <n-card title="可排序工作流">
      ...
      <n-alert type="info" style="margin:12px 0">拖动工作流行即可调整执行顺序，越靠上越先执行。</n-alert>
      <n-data-table data-testid="workflow-priority-table" :columns="workflowColumns" :data="workflows" :loading="workflowReordering" :row-props="workflowRowProps" />
    </n-card>
...
import { NAlert, NButton, NCard, NCode, NDataTable, NForm, NFormItem, NFormItemGi, NGrid, NH1, NInput, NSelect, NSpace, NTag, NTooltip, useMessage } from 'naive-ui'
...
const message=useMessage();const mode=ref('global_pool');const models=ref<any[]>([]);const workflows=ref<any[]>([]);const upstream=ref<any[]>([]);const unified=ref<any[]>([]);const loadingUpstream=ref(false);const capabilitySource=ref('');const draggedModelId=ref<number|null>(null);const draggedWorkflowId=ref<number|null>(null);const modelReordering=ref(false);const workflowReordering=ref(false)
...
async function addWorkflow(){let steps;try{steps=JSON.parse(workflowForm.steps);if(!Array.isArray(steps))throw new Error()}catch{return message.error('有序步骤必须是 JSON 数组')};await postJson('/api/admin/auxiliary/workflows',{workflow_type:workflowForm.workflow_type,scope:mode.value==='per_unified_model'?'unified_model':'global',unified_model_id:mode.value==='per_unified_model'?workflowForm.unified_model_id:null,input_capability:workflowForm.input_capability,output_capability:workflowForm.output_capability,ordered_steps:steps});await load()}
...
async function reorderModels(targetId:number){const sourceId=draggedModelId.value;draggedModelId.value=null;if(sourceId===null||sourceId===targetId)return;const ordered=[...models.value];const sourceIndex=ordered.findIndex(row=>row.id===sourceId);const targetIndex=ordered.findIndex(row=>row.id===targetId);if(sourceIndex<0||targetIndex<0)return;const [moved]=ordered.splice(sourceIndex,1);ordered.splice(targetIndex,0,moved);models.value=ordered.map((row,index)=>({...row,priority:index+1}));modelReordering.value=true;try{await patchJson('/api/admin/auxiliary/models/reorder',{ids:ordered.map(row=>row.id)});message.success('辅助模型优先级已更新')}catch(error){message.error(String(error));await load()}finally{modelReordering.value=false}}
async function reorderWorkflows(targetId:number){const sourceId=draggedWorkflowId.value;draggedWorkflowId.value=null;if(sourceId===null||sourceId===targetId)return;const ordered=[...workflows.value];const sourceIndex=ordered.findIndex(row=>row.id===sourceId);const targetIndex=ordered.findIndex(row=>row.id===targetId);if(sourceIndex<0||targetIndex<0)return;const [moved]=ordered.splice(sourceIndex,1);ordered.splice(targetIndex,0,moved);workflows.value=ordered.map((row,index)=>({...row,priority:index+1}));workflowReordering.value=true;try{await patchJson('/api/admin/auxiliary/workflows/reorder',{ids:ordered.map(row=>row.id)});message.success('工作流顺序已更新')}catch(error){message.error(String(error));await load()}finally{workflowReordering.value=false}}
function sortableRowProps(row:any,kind:'model'|'workflow'){const draggedId=kind==='model'?draggedModelId:draggedWorkflowId;const busy=kind==='model'?modelReordering:workflowReordering;return {draggable:!busy.value,class:`sortable-row${draggedId.value===row.id?' is-dragging':''}`,onDragstart:(event:DragEvent)=>{draggedId.value=row.id;if(event.dataTransfer){event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',String(row.id))}},onDragover:(event:DragEvent)=>{event.preventDefault();if(event.dataTransfer)event.dataTransfer.dropEffect='move'},onDrop:(event:DragEvent)=>{event.preventDefault();void (kind==='model'?reorderModels(row.id):reorderWorkflows(row.id))},onDragend:()=>{draggedId.value=null}}}
function modelRowProps(row:any){return sortableRowProps(row,'model')}
function workflowRowProps(row:any){return sortableRowProps(row,'workflow')}
...
const orderColumn={title:'优先级',key:'priority',width:76,align:'center',render:(_:any,index:number)=>h('span',{class:'priority-order',title:'拖动调整优先级','aria-label':`当前优先级 ${index+1}，拖动可调整`},[h('span',{class:'drag-handle','aria-hidden':'true'},'⠿'),h('span',{},String(index+1))])}
const modelColumns:any[]=[orderColumn,{title:'供应商',key:'provider',render:(r:any)=>r.provider_instance?.name||'-'},{title:'上游模型',key:'upstream',render:(r:any)=>r.upstream_model?.display_name||r.upstream_model?.model_id||r.upstream_model_id},{title:'统一模型',key:'unified_model',render:(r:any)=>r.unified_model?.name||'全局池'},{title:'能力',key:'capabilities',render:(r:any)=>(r.capabilities||[]).join(', ')},{title:'状态',key:'enabled',render:(r:any)=>h(NTag,{type:r.enabled?'success':'default'},{default:()=>r.enabled?'启用':'停用'})},{title:'操作',key:'actions',render:actions(toggleModel,removeModel)}]
const workflowColumns:any[]=[{...orderColumn,title:'顺序'},{title:'工作流',key:'workflow_type',render:(r:any)=>workflowLabels[r.workflow_type]||r.workflow_type},{title:'输入 → 输出',key:'flow',render:(r:any)=>`${r.input_capability} → ${r.output_capability}`},{title:'步骤数',key:'ordered_steps',render:(r:any)=>(r.ordered_steps||[]).length},{title:'状态',key:'enabled',render:(r:any)=>h(NTag,{type:r.enabled?'success':'default'},{default:()=>r.enabled?'启用':'停用'})},{title:'操作',key:'actions',render:actions(toggleWorkflow,removeWorkflow)}]
onMounted(load)
</script>
<style scoped>
.model-preview{color:var(--n-text-color-3);font-size:12px;overflow-wrap:anywhere}
:deep(.sortable-row){cursor:grab;transition:background-color .15s ease,opacity .15s ease}
:deep(.sortable-row:hover){background:rgba(24,160,88,.06)}
:deep(.sortable-row.is-dragging){opacity:.45;cursor:grabbing}
.priority-order{display:inline-flex;align-items:center;gap:6px;user-select:none}
.drag-handle{color:#888;font-size:18px;line-height:1}
</style>
