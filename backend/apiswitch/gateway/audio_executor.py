"""Unified routing, audit and accounting for OpenAI-compatible audio APIs."""
...
from apiswitch.schemas.gateway import AudioSpeechRequest, AudioTranscriptionRequest
...
AudioRequest = AudioSpeechRequest | AudioTranscriptionRequest
...
async def _execute_audio(
    ...
    *,
    ...
    input_for_estimate: object,
    invoke: Callable[[ProviderAdapter, AudioRequest], Awaitable[Result]],
    ...
    attempts: list[dict] = []
    estimated_input_tokens = estimate_token_count(input_for_estimate)
    log = RequestLog(
    ...
    try:
        ...
        for selected in candidates:
            ...
            try:
                ...
                result = await invoke(provider, upstream_request)
                ...
                log.input_tokens = estimated_input_tokens
                log.output_tokens = 0
...
async def execute_audio_speech(request: AudioSpeechRequest, db: Session, **kwargs: object):
    return await _execute_audio(
        request,
        db,
        inbound_protocol="openai_audio_speech",
        input_for_estimate=request.input,
        invoke=lambda provider, payload: provider.audio_speech(payload),  # type: ignore[arg-type]
        **kwargs,
    )


async def execute_audio_transcription(request: AudioTranscriptionRequest, db: Session, **kwargs: object):
    return await _execute_audio(
        request,
        db,
        inbound_protocol="openai_audio_transcriptions",
        input_for_estimate={"filename": request.filename, "bytes": len(request.file_bytes), "prompt": request.prompt},
        invoke=lambda provider, payload: provider.audio_transcriptions(payload),  # type: ignore[arg-type]
        **kwargs,
    )
