"""Unified-model execution for OpenAI-compatible image generation."""
...
from apiswitch.schemas.gateway import ImageEditRequest, ImageGenerationRequest, ImageVariationRequest
...
async def execute_image_generations(
    request: ImageGenerationRequest | ImageEditRequest | ImageVariationRequest,
    db: Session,
    *,
    ...
    attempts: list[dict] = []
    estimated_input_tokens = estimate_token_count(getattr(request, "prompt", "image variation"))
    log = RequestLog(
        request_id=request_id,
        started_at=started_at,
        inbound_protocol=f"openai_images_{operation}",
    ...
    try:
        ...
        for selected in candidates:
            ...
            try:
                ...
                method = getattr(provider, f"image_{operation}")
                response = await method(request.model_copy(update={"model": selected.upstream_model}))
...
async def execute_image_edit(
    ...
    **kwargs: object,
    ...
    return await execute_image_generations(request, db, operation="edits", **kwargs)


async def execute_image_variation(
    request: ImageVariationRequest,
    db: Session,
    **kwargs: object,
) -> dict:
    return await execute_image_generations(request, db, operation="variations", **kwargs)
