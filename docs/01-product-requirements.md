...
API Token 是客户端访问 APISwitch 的凭据，与供应商 API Key 完全分离。支持名称、权限范围、有效期、启停、最后使用时间和预算归属。明文只在创建时返回一次；页面同时显示并支持一键复制当前网关地址。
...
- 系统设置包含端口、目录、自启动、上传限制、备份恢复、密钥、系统信息和 WebDAV，通过 `/settings` 高级页面直达，不进入左侧主导航。
