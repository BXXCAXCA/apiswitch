# APISwitch ChatGPT 开发交接文档（历史快照，请勿作为当前依据）

> 本文件记录的是 2026-07-23 的旧分支快照。它提及的分支、提交、测试数量、Release 和待办均可能过时。
>
> 在 GPT 网页版继续开发时，请改用 [CONTINUE_IN_GPT.md](CONTINUE_IN_GPT.md)；该文件是当前唯一交接入口。本文件仅为保留历史背景而存在。
...
辅助步骤成本进入预算上限与执行动作。现有实现已经提供逐步骤独立日志、父请求关联、Token、成本、延迟、失败阶段和用量聚合。
